...
| 用户-角色 (user_role) | 用户与角色多对多，可带 validFrom/validTo | `/api/perm/user-roles` |
| 角色-资源-操作 (role_resource_permission) | 角色对某资源在某操作上的授权 | `/api/perm/role-permissions` |
...
| 变更记录 (permission_change_log) | 变更及影响用户/角色 | `/api/perm/change-logs`、`/api/perm/change-logs/page` |
...
| 用户-角色 | 分配/回收角色、有效期 | `/api/perm/user-roles` | list、assign、revoke |
| 角色-权限 | 为角色配置资源+操作、可管理、条件 | `/api/perm/role-permissions` | list、add、remove |
...
| 变更记录 | 按用户/角色/时间等查询 | `/api/perm/change-logs`、`/api/perm/change-logs/page` | change-logs、change-logs/page |
